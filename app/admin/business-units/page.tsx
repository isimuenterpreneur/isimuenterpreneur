...
import type { BusinessUnit } from "../../../types/firebase";
import DriveImageUploader from "../../../components/DriveImageUploader";

const initial: Omit<BusinessUnit, "id"> = { name: "", description: "", image: "" };
    ...
    <AdminLayout>
      <div className="space-y-8">
        <div className="rounded-3xl border border-white/10 bg-black/70 p-8 shadow-soft">
          <h1 className="text-3xl font-semibold text-white">Kelola Unit Usaha</h1>
          <p className="mt-3 text-slate-400">Tambah atau hapus unit usaha dengan cepat lewat dashboard.</p>
        </div>
        <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="overflow-hidden rounded-3xl border border-white/10 bg-slate-950 p-6 shadow-soft">
            <div className="flex items-center justify-between gap-4 pb-6">
              <h2 className="text-xl font-semibold text-white">Daftar Unit Usaha</h2>
                ...
                <div key={item.id} className="rounded-3xl border border-white/10 bg-slate-900 p-5 text-white">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <button
                      ...
                      onClick={async () => {
                        if (confirm("Hapus unit usaha ini?")) {
                          await deleteRecord(`business_units/${item.id}`);
                        }
                      }}
                    >
                      Hapus
          ...
          <div className="rounded-3xl border border-white/10 bg-black/70 p-6 shadow-soft">
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-4">
              ...
              <div className="space-y-3">
                <span className="text-sm text-slate-200">Gambar Unggulan</span>
                <DriveImageUploader onUpload={handleImageUpload} />
